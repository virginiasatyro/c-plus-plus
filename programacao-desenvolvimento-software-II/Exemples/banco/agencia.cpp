...
Agencia::Agencia(int numero, std::string logradouro, std::string bairro, std::string cidade, int cep){
    endereco_t endereco;
...
Cliente &Agencia::adiciona_cliente(int cpf, std::string nome){
    if(_clientes.count(cpf) != 0){
        throw std::invalid_argument("Cliente ja existe!\n");
    }
...
Conta &Agencia::cria_conta(int cpf){
    if(_clientes.count(cpf) == 0){
        throw std::invalid_argument("Cliente nao existe!\n");
    }
